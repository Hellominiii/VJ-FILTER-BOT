...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('🍿Mᴏᴠɪᴇ Gʀᴏᴜᴘ🍿', url='https://t.me/searchmoviehere')
        ],[
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            return
        btn = [[
            InlineKeyboardButton("♦️Jᴏɪɴ Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ♦️", url=invite_link.invite_link)
        ]]
            ...
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
                    try:
                        ...
                        btn.append([InlineKeyboardButton("♻️ Tʀʏ Aɢᴀɪɴ", callback_data=f"checksub#{kk}#{file_id}")])
                    except (IndexError, ValueError):
                        btn.append([InlineKeyboardButton("♻️ Tʀʏ Aɢᴀɪɴ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
            else:
                try:
                    kk, file_id = message.command[1].split("_", 1)
                    btn.append([InlineKeyboardButton("♻️ Tʀʏ Aɢᴀɪɴ", callback_data=f"checksub#{kk}#{file_id}")])
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('♦️Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ♦️', url=f'https://t.me/{SUPPORT_CHAT}'),
                        InlineKeyboardButton('♦️Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ♦️', url=CHNL_LNK)
                    ],[
                        InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/jain_shabb")
                    ],[
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/jain_shabb")
                ],[
                ...
                button = [[
                    InlineKeyboardButton('🔺Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ♦️', url=f'https://t.me/{SUPPORT_CHAT}'),
                    InlineKeyboardButton('♦️Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ♦️', url=CHNL_LNK)
                ],[
                    InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/jain_shabb")
                ]]
    ...
    if not files_:
        ...
        try:
            ...
            title = '@jain_shabb  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@jain_shabb  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@jain_shabb  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("♻️Verify♻️", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
